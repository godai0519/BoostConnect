...
void http_session::do_read()
{
    ...
    boost::asio::async_read_until(*socket_, *read_buf,
        "\r\n\r\n",
        [this, self, read_buf](const error_code ec, const size_t size)
        {
            if(ec) return;
            ...
            if( byte != 0 )
            {
                ...
                boost::asio::async_read(
                    ...
                    *read_buf,
                    ...
                    [this, self, read_buf, request](const error_code ec, const size_t size)
                    {
                        if(!ec) handle_read(read_buf, request);
                    }
                );
            }
            else
            {
                //長さがない or 0 -> 読まずに
                handle_read(read_buf, request);
            }
        }
    );
}

void http_session::handle_read(const boost::shared_ptr<boost::asio::streambuf> read_buf, const boost::shared_ptr<bstcon::request> request)
{
