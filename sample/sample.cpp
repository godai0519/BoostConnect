...
{    
    ...
    bstcon::client::connection_ptr connection = client(
        "www.hatena.ne.jp",
        [&connection,request_buf](bstcon::client::connection_ptr, boost::system::error_code ec)->void
        {
            if(ec) return;
            connection->send(
                request_buf,
    ...
    //{
    //        boost::shared_ptr<bstcon::connection_type::connection_base> connection_w = client(
    //            "www.hatena.ne.jp",
    //            request_buf,
    //            [](const bstcon::client::connection_ptr response_tmp,const boost::system::error_code&)->void{std::cout << "Connection End" << std::endl;}
    //        );
    //        //connection_w.lock().reset();

    //        {
    //                io_service.run();

    //                if(boost::shared_ptr<bstcon::connection_type::connection_base> connection = ((boost::weak_ptr<bstcon::connection_type::connection_base>)connection_w).lock())
    //                {
