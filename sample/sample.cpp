...
//    );
//
//  boost::shared_ptr<bstcon::response> response;
//  client(
//    "www.hatena.ne.jp",
//    request_buf,
//    [&response](const boost::shared_ptr<bstcon::response> response_tmp,const boost::system::error_code&)->void{response = response_tmp;std::cout << "Connection End" << std::endl;}
//  );
//
...
int main()
{
  ...
  service.start(
    ...
    {
      for(auto it = req.header.cbegin();it != req.header.cend();++it)
      {
